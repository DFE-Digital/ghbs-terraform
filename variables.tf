...
variable "postgresql_network_connectivity_method" {
  type        = string
  description = "Specify postgres networking method, public or private. See https://learn.microsoft.com/en-gb/azure/postgresql/flexible-server/concepts-networking"
  default     = "public"
  validation {
...
variable "postgresql_server_version" {
  ...
  default     = "11"
  validation {
    condition     = contains(["11", "12", "13", "14"], var.postgresql_server_version)
    error_message = "Valid values for postgresql_server_version are (11, 12, 13, 14)."
  }
}
...
variable "postgresql_max_storage_mb" {
  ...
  default     = 32768
  validation {
    condition     = contains([32768, 65536, 131072, 262144, 524288, 1048576, 2097152, 4194304, 8388608, 16777216], var.postgresql_max_storage_mb)
    error_message = "Valid values for postgresql_max_storage_mb are (32768, 65536, 131072, 262144, 524288, 1048576, 2097152, 4194304, 8388608, 16777216)."
  }
}
...
variable "postgresql_enabled_extensions" {
  type        = string
  description = "Specify a comma seperated list of Postgres extensions to enable. NOTE: this will only allow you to enable the extension with SQL. If you do not list an extension here, you will not be able to enable it with SQL. See https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/concepts-extensions#postgres-14-extensions"
  default     = ""
}

variable "postgresql_public_firewall_rules" {
  type = map(object({
    start_ip_address = string
    end_ip_address   = string
  }))
  description = "List of IP address ranges to add into the postgres firewall. Note: only applicable if postgresql_enable_vnet_integration is set to false."
  default     = {}
}
